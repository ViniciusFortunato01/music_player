...
function arte(){
    var entrada = document.getElementById('c-input');
    var contador = document.getElementById('contador');

    if(contador.value <= 100){
        for(let i = 0; i<contador.value; i++){
            let signal = Math.round(Math.random()) == 0 ? -1 : 1;
            let cValue = signal*(Math.random()*4);
            entrada.value = cValue;
            entrada.focus();
            entrada.dispatchEvent(new KeyboardEvent('keypress', {
                key: "Enter",
                keyCode: 13, 
                code: "Enter", 
                shiftKey: false, 
